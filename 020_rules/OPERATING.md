...
in `../010_soul/README.md`: character on top, rules underneath, loaded together every session. In a
...
an adversarial verifier (`030_agents/defs/harvey.md`) dispatched on a deliverable *before* it is
...
- **Two triggers, one capability.** The gate (dispatch per deliverable, synchronous) and the
  patrol (`030_agents/patrol/`, a daily sweep re-proving work already claimed done, max 3
  receipt-backed findings to a persistent queue). Build the gate first; the patrol is nearly free
  once it exists.

## Dispatch

The full doctrine lives in `../030_agents/README.md`. The one-line version: default to dispatching
